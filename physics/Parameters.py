...
def CreateFisherKolmprm(filename, conv = False):
	...
	prm = parametersFisherKolm(conv)

	if pfh.generateprmfile(prm,filename) == 1:

		Lines = pfh.readlinesprmfile(filename)
		commentedlines = commentingprmFisherKolm(Lines)
...
def parametersFisherKolm(conv):
	...
	prm['Model Parameters']['Diffusion']=0.1
	prm['Model Parameters']['c or l']='c'
	prm['Model Parameters']['N or D']='N'

	prm['Model Parameters']['Isotropic Diffusion'] = 'Yes'

	prm['Model Parameters']['alpha'] = 0.25
	
	prm['Model Parameters']['gamma'] = 10
	
	prm['Model Parameters'].add_subsection('Forcing Terms')

	prm['Model Parameters']['Forcing Terms'] = '0*x[0]'
	...
	prm['Model Parameters']['Initial Condition '] = '0*x[0]'
	...
	if (conv):
		...
		prm['Convergence Test']['Exact Solution'] = '0*x[0]'
...
def commentingprmFisherKolm(Lines):
	...
	for line in Lines:
		...
		if not (line.find("set Diffusion") == -1):
			comm = comm + "# Diffusion Constant of the Fluid Network [m^2/years]"

		if not (line.find("set alpha") == -1):
			comm = comm + "#  alpha"
